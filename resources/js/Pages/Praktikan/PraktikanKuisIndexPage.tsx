...
export default function PraktikanKuisIndexPage({
    auth,
    currentDate,
    kuis,
}: PageProps<{
    ...
    const [startKuisForm, setStartKuisForm] =
        useState<StartKuisForm>(startKuisFormInit);
    const RenderDeskripsiSoal = ({ deskripsi }: { deskripsi: string }) => {
        const deltaDeskripsi = deltaParse(deskripsi);
        return <RenderQuillDelta delta={deltaDeskripsi} />;
    };

    const handleStartKuisFormOpen = (
        open: boolean,
        startKuisFormProps: StartKuisForm = startKuisFormInit
    ) => {
    ...
    const handleStartKuisFormSubmit = (event: FormEvent<HTMLFormElement>) => {
        ...
        axios
            ...
            .then((res) => {
                ...
                router.visit(
                    route("praktikan.kuis.exam", {
                        id: res.data.kuis_praktikan_id,
                    })
                );
            })
            .catch((err: unknown) => {
    ...
    const handleStartKuisClick = (kuis: Kuis) => {
        if (kuis.kuis_praktikan_id) {
            router.visit(
                route("praktikan.kuis.exam", { id: kuis.kuis_praktikan_id })
            );
            return;
        }
    ...
    return (
        <>
            <PraktikanLayout auth={auth}>
                        ...
                        >
                            <CardHeader>
                                <div className="flex justify-between items-start min-h-28">
                                    <CardTitle className="text-xl line-clamp-4 text-ellipsis">
                                        {kuis.nama}
                                    </CardTitle>
                                </div>
                                ...
                                <Button
                                    className={`w-full ${
                                        kuis.selesai
                                            ...
                                            : "bg-blue-600 hover:bg-blue-600/85"
                                    }`}
                                    disabled={
                                        kuis.selesai
                                            ? true
                        ...
                        <div className="flex flex-col items-center justify-center text-center space-y-4">
                            ...
                            <div className="space-y-2">
                                <h3 className="text-xl font-semibold tracking-tight">
                                    Horee🥳 Tidak ada kuis mendatang
                                </h3>
                                <p className="text-sm text-muted-foreground max-w-md mx-auto">
                                    Saat ini tidak ada kuis yang dijadwalkan
                                    untuk Anda. Kuis baru akan muncul di sini
                                    ketika tersedia.
                                </p>
                            </div>
                            <div className="pt-4">
                ...
                <AlertDialog
                    open={openStartKuisForm}
                    onOpenChange={handleStartKuisFormOpen}
                >
                    <AlertDialogContent
                    ...
                    >
                                ...
                                <div className="flex flex-col justify-between flex-wrap gap-4 text-sm">
                                    <div className="flex items-center">
                                        ...
                                        <span>
                                            Durasi :{" "}
                                            {kuisDuration(
                                                startKuisForm.waktu_mulai,
                                                startKuisForm.waktu_selesai
                                            )}{" "}
                                            Menit
                                        </span>
                                    </div>
                        ...
                        <form
                            ...
                            onSubmit={handleStartKuisFormSubmit}
                        >
                            <div className="grid gap-2">
                                ...
                                {startKuisForm.has_kode ? (
                                    <p className="text-sm text-primary">
                                        Masukkan kode kuis untuk melanjutkan
                                    </p>
                                ) : (
                                    <p>
