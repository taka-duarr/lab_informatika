...
//        dd($queryPraktikum);
    ...
    public function kuisResultPage(Request $request, $id)
    {
        $authAdmin = Auth::guard('admin')->user();
        if (!$authAdmin) {
            abort(401);
        ...
        try {
            return Inertia::render('Admin/kuis/result/admin-kuis-result-index', [
                'kuis' => fn() => [
                    'id' => $kuis->id,
                    'nama' => $kuis->nama,
                    'praktikans' => $kuis->praktikans->map(function ($praktikan) {
                        return [
                            'id' => $praktikan->id,
                            'nama' => $praktikan->nama,
